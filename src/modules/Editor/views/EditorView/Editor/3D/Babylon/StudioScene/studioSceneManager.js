...
import { SideTypes } from '../../../utilities';
...
export default class StudioSceneManager {
	...
	createScene(onFinish, configData, threeD) {
		...
		this.selectedModelConfig = selectedModelType;
	...
	createCamera() {
		this.mainCamera = new BABYLON.ArcRotateCamera('ArcCamera', 1.45, 1.4, 27, new BABYLON.Vector3(0, 9, -0.7), this.scene);
		...
		this.staticCamera = new BABYLON.ArcRotateCamera('staticCamera', 2.7, 1.4, 20, new BABYLON.Vector3(0, 6.5, 0), this.scene);
	}
	setUpEnvironMent() {
		const dirLight = new BABYLON.DirectionalLight('DirectionalLight', new BABYLON.Vector3(0, -1, 0.3), this.scene);
		...
		backgroundMaterial.diffuseTexture = new BABYLON.Texture(`${window.location.origin}/Textuers/scene/backgroundGround.png`, this.scene);
		...
		//Create CubicTexture
		const skyboxCubecTexture = BABYLON.CubeTexture.CreateFromPrefilteredData('./environment/environment.env', this.scene);
	...
	changeSide(newSide) {
		...
		const sideConfigs = this.selectedModelConfig?.sides.find((side) => side.id === this.selectedSide)?.configs;
		if (sideConfigs) {
	...
	assignScaleAnimation(targetObj) {
		...
		keys.push({
			frame: 50,
			value: new BABYLON.Vector3(targetObj.scaling.x * 1.35, targetObj.scaling.y * 1.35, targetObj.scaling.z * 1.35),
		});
	...
	simplifyRadians(radians) {
		...
		return simplifiedRadians < 0 ? simplifiedRadians + BABYLON.Tools.ToRadians(360) : simplifiedRadians;
	}
	//#endregion
}
