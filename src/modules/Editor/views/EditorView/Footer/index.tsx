...
function Footer() {
	...
	const isSubMenu = useMemo(() => {
		if (selectedCategory === 'Texts' || selectedCategory === 'Draw' || selectedCategory === 'Graphics') {
			return true;
		} else {
			return false;
		}
	...
	return (
		<Box className={classes.footer}>
			{(width ?? 0) > 700 && (height ?? 0) > 450 && (
				<Box alignItems={'flex-end'} style={{ pointerEvents: 'none', marginTop: '100%' }}>
					<Box className={classes.priceWrapper}>
						<Box
							...
							<IconButton size='small' aria-label='menu' color='inherit' aria-controls='notification-menu' aria-haspopup='true'>
			...
			<Box display={'flex'} flexDirection={selectedCategory === 'Draw' ? 'column' : 'row'} className={classes.bottomMenuWrapper}>
				{/* {Draw Menu} */}
				{(width ?? 0) < 700 && (height ?? 0) > 450 && (
					<Box display={'flex'} className={clsx(classes.subBottomMenuWrapper, { isSubMenu: true })}>
						<Box
							...
							{selectedCategory && isSubMenu && (selectedCategory === 'Draw' || getSelectedObjects().length) && (
								<Box
									width={selectedCategory === 'Draw' ? '650px' : selectedCategory === 'Graphics' ? '780px' : '840px'}
									className={clsx(classes.subActionMenu, { isSubMenu: true })}>
									<SubFooter />
								</Box>
							)}
						</Box>
			...
			{(selectedCategory === 'Draw' || !showRightMenu) && (width ?? 0) > 650 && (height ?? 0) < 450 && (
				<Box display={'flex'} className={classes.subBottomMenuWrapper}>
					<Box
						display={bottomMenu === 'CircularMenu' && ((width ?? 0) <= 700 || (height ?? 0) <= 450) ? 'none' : 'flex'}
						alignItems={(width ?? 0) > 700 && (height ?? 0) > 450 ? 'center' : 'flex-start'}
						flexDirection={'column'}
